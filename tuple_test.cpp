...
// static_assert(!explicit_test<std::vector<int>>()); // Wrong test?
...
// static_assert(!explicit_conversion_test<std::vector<int>>()); // Wrong test?
...
void test_tuple() {
  ...
  static_assert(std::is_constructible_v<Tuple<int&&, char&>, int&&, char&>);
  int r = 1;
  Tuple<int&, char&&> tu(r, 'r');
  // static_assert(tu.test<int&, char&>());
  int a1 = 1;
  char a2 = '2';
  int& a3 = a1;
  char& a4 = a2;
  // Tuple<int&, char&&> tu_(a3, a4);
  // static_assert(std::is_constructible_v<char&&, char&>);
  static_assert(!std::is_constructible_v<Tuple<int&, char&&>, int&, char&>);
  static_assert(std::is_constructible_v<Tuple<int&, const char&>, int&, char&&>);
...

  {
    ...
    std::cout << Accountant::move_constructed << ' ';
...
//  // Incorrect test:
//  int ndx = 1;
//  Tuple<int&, NeitherDefaultNorCopyConstructible> nd1(ndx, 1.);
//  Tuple<int&, NeitherDefaultNorCopyConstructible> nd2(std::move(nd1));
